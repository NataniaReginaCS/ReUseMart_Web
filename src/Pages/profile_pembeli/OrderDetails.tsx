...
import { fetchOrderDetailsById, fetchOrderHistoryById } from '../../api/ApiTransaksi'; // You need to make this API
                ...
                <div className="flex flex-col w-full min-h-[500px] mt-5 border-1 border-gray-300 rounded-lg">
                    <div className="flex items-center w-full justify-between ">
                        <div className="flex items-center">
                            ...
                            <p className="text-md font-semibold text-[#4D4D4D]  ml-2 mt-5">• {pembelian?.tanggal_lunas ? new Date(pembelian.tanggal_lunas).toLocaleDateString('en-US', {
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
                            }) : ""}  • 3 Products</p>
                        ...
                        <p className="text-lg font-semibold text-[#00B207]  mr-8 mt-5 hover:underline cursor-pointer  " onClick={() => navigate("/order")}>Back to List</p>
                    </div>

                    <div className="flex gap-4">
                        <div className="flex flex-col w-full max-w-[400px] mt-5 p-4 m-4 border-1 border-gray-300 rounded-lg">
                            <p className="text-md font-semibold text-[#999999]">BILLING ADDRESS</p>
                            <hr className="-mx-4 my-2 border-t border-gray-300" />
                            <p className="text-lg font-semibold">{profile?.nama}</p>
                            <p className="text=[#999999]">{alamatUtama?.nama_alamat || "Alamat masih kosong"}</p>
                        ...
                        <div className="flex flex-col w-full max-w-[350px] mt-5 p-4 m-4 border border-gray-300 rounded-lg">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-md font-semibold">{pembelian?.id_pembelian}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between">
                                ...
                                <p className="text-md text-green-500">
                                    Rp {((pembelian?.total || 0) + (pembelian?.ongkir || 0)).toLocaleString("id-ID")}
                                </p>
                            </div>
                        </div>
                    ...
                    <table className="w-full mt-5 mb-5 border border-gray-200">
                        ...
                        <tbody>
                            {barang?.map((item) => (
                                <tr key={item.id_barang} className="text-center border-b hover:bg-gray-50">
