...
import { FetchPenitip } from "../../../api/ApiAdmin";
import { SyncLoader } from "react-spinners";
import { MdChevronLeft, MdChevronRight } from "react-icons/md";
import ModalEditPenitip from "./ModalEditPenitip";
import ModalDeletePenitip from "./ModalDeletePenitip";
import ModalAddPenitip from "./ModalAddPenitip";
...
const CSPenitip = () => {
    ...
    const handleEditClick = (data: Penitip) => {
        setSelectedPenitip(data);
        setShowModal(true);
    };

    const handleDeleteClick = (data: Penitip) => {
        setSelectedPenitip(data);
    ...
    const filteredData = data.filter(
        (penitip) =>
            ...
            penitip.no_ktp.toString().includes(searchTerm.toLowerCase())
    );
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full min-w-max table-auto text-left">
                                ...
                                <tbody className="text-center">
                                    {currentData.map((penitip: any, index) => {
                                        ...
                                        return (
                                            <tr key={penitip.id_penitip}>
                                                <td className={classes}>
                                                    <p className="font-normal">{penitip.nama}</p>
                                                </td>
                                                <td className={classes}>
                                                    <p className="font-normal">{penitip.email}</p>
                                                </td>
                                                <td className={classes}>
                                                    <p className="font-normal">{penitip.telepon}</p>
                                                </td>
                                                <td className={classes}>
                                                    <p className="font-normal">{penitip.no_ktp}</p>
                                                </td>
                                                <td className={classes}>
                                                    <div className="flex flex-row justify-evenly gap-2">
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => handleEditClick(penitip)}
                                                        >
                                                        ...
                                                        <button className="font-medium bg-red-500 text-white rounded-3xl w-30 flex cursor-pointer text-center items-center justify-center gap-1 p-1"
                                                            onClick={() => handleDeleteClick(penitip)}
                                                        >
